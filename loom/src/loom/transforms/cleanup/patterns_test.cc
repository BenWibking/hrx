...
TEST(CleanupPatternsTest, ConfiguredSourceProvidersCoverOwnedRoots) {
  const loom_cleanup_pattern_provider_set_t* provider_set =
      loom_cleanup_configured_pattern_provider_set();
  EXPECT_EQ(provider_set->universal_pre_fold.count, 0u);
  EXPECT_EQ(provider_set->universal_post_type.count, 0u);
  ...
  const loom_rewrite_pattern_registry_t* registry =
      loom_cleanup_pattern_registry_storage_registry(&storage)->source_combine;
  EXPECT_EQ(
      loom_rewrite_pattern_registry_lookup_kind(registry, LOOM_OP_SCF_SELECT)
          .count,
      1u);
  EXPECT_EQ(
      loom_rewrite_pattern_registry_lookup_kind(registry, LOOM_OP_VIEW_LOAD)
          .count,
      1u);
  EXPECT_EQ(loom_rewrite_pattern_registry_lookup_kind(
                registry, LOOM_OP_VECTOR_FROM_ELEMENTS)
                .count,
            2u);
  EXPECT_EQ(loom_rewrite_pattern_registry_lookup_kind(
                registry, LOOM_OP_VECTOR_TABLE_LOOKUP)
                .count,
            1u);
  EXPECT_EQ(registry->pattern_count, 5u);
  loom_cleanup_pattern_registry_storage_deinitialize(&storage);
}
